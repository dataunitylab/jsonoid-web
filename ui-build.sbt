...
// Execute on commandline, depending on the operating system. Used to execute
// yarn commands.
...
// Execute `yarn install` command to install all node module dependencies. Return Success if already installed.
...
Docker / stage := ((Docker / stage) dependsOn `ui-prod-build`).value
